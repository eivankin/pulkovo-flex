...
if __name__ == '__main__':
    ...
    paths = glob(os.path.dirname(os.path.abspath(__file__)) + '/Преподы, аудитории, параметры программ.xlsx', recursive=True)
    ...
    Excel = pd.ExcelWriter('ExcelProf.xlsx')
    for sheet_id in range(len(bigdata)):
        bigdata[sheet_id].to_excel(excel_writer=Excel ,sheet_name='Page '+ str(sheet_id))
    Excel.save()