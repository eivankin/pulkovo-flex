...
if __name__ == '__main__':
    ...
    paths = glob(os.path.dirname(os.path.abspath(__file__)) + '/Смены.xlsx', recursive=True)
    ...
    Excel = pd.ExcelWriter('ExcelWorkShifts.xlsx')
    for sheet_id in range(len(bigdata)):
        bigdata[sheet_id].to_excel(excel_writer=Excel ,sheet_name='Graph '+ str(sheet_id))
    Excel.save()